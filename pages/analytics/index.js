...
export class AnalyticsPage {
  ...
  render() {
    ...
    header.render(); // Кнопка "Домой" теперь в хедере

    this.parent.insertAdjacentHTML("beforeend", this.getHTML());

    const sampleItems = ["Nike AF1", "Apple AirPods", "Levi’s Jeans"];
    const delimiter = " | ";
    document.getElementById("concat-items").textContent = formatProductFeatures(sampleItems, delimiter);
    ...
    anagramButton.addEventListener("click", () => {
      ...
      anagramResult.innerHTML = (groups.length === 0)
        ? "Нет групп из 2+ слов."
        : groups.map((group) => `[${group.join(", ")}]`).join("<br/>");
    });
  }
}
